...
contract Pool is PoolStorage, IPool {
    ...
    function underwrite(
        ...
        //commented out by Sing as it is not compiling correctly// LoanAgent loanAgent = new LoanAgent{salt: abi.encodePacked(uint40(block.timestamp))}(owner, amount);

        // switch miner actor's owner to LoanAgent contract address. 
        //commented out by Sing as it is not compiling correctly// MinerAPI(owner).change_owner_address(address(loanAgent));
        //commented out by Sing as it is not compiling correctly// return address(loanAgent);
    }
